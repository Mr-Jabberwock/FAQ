<template>
    <div class="editor-toolbar">
        <ul class="nav">
            <li v-for="(tool, ind) in tools" :key="ind" class="nav-item">
                ...
                <div v-if="tool.type === 'toggle'">
                    <span v-for="(button, order) in tools.buttons" :key="order">
                        <button v-if="button.icon != null" v-html="button.icon" :title="button.text" type="button" class="button" @click="toggleButton(ind, button, order)"></button>
                        <button type="button" class="button" @click="toggleButton(ind, button, order)">
                            {{ button.text}}
                        </button>
                    </span>
                </div>
                <div v-if="tool.type === 'wrap'">
                    <span v-for="(button, order) in tools.buttons" :key="order">
                        <button v-if="button.icon != null" v-html="button.icon" :title="button.text" type="button" class="button" @click="wrapButton(ind, button)"></button>
                        <button type="button" class="button" @click="wrapButton(ind, button)">
                            {{ button.text}}
                        </button>
                    </span>
                </div>
                <div v-if="tool.type === 'button'">
                    <button v-if="tool.icon != null" v-html="tool.icon" :title="tool.text" type="button" class="button" @click="singleButton(tool.action)"></button>
                    <button type="button" class="button" @click="singleButton(tool.action)">
                        {{ button.text}}
                    </button>
                </div>
    ...
    data() {
        ...
        toggleButton(index, button, order) {
            ...
            if(toggleAction.includes(button.value)) {
                toggleAction.splice(order, 1)
            }else {
                toggleAction.push(button.value)
            }
            this.$emit('update:toggle', toggleAction)
        },
